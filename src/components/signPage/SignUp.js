...
export default function SignUp() {
    ...
    function signup(e) {
        ...
        if (pictureUrl.match(/\.(jpeg|jpg|gif|png|svg)$/) != null){
            ...
            request.catch((error)=> {
                if(error.response.status === 403) { alert('Este email já está cadastrado!') }
                else { alert('Algo deu errado. Por favor, tente novamente!')}
                setIsLoading(false);
            })
        } else { alert("Please, provide a valid picture URL") }
    }
    ...
    return(
        <Form onSubmit={signup}>
            ...
            <input onChange={(e)=>setPictureUrl(e.target.value)} value={pictureUrl} type="text" placeholder="picture url" required></input>
